...
int main(int argc, char* argv[]) {
  ...
  if (postfit) {
    ...
    if(total_shapes){
      ...
      if (skip_uncertainties) {
          std::cout << ">> Doing postfit (no uncertainties): TotalBkg" << std::endl;
          post_shapes_tot["TotalBkg"] = cmb_bkgs.GetShape();
          std::cout << ">> Doing postfit (no uncertainties): TotalSig" << std::endl;
          post_shapes_tot["TotalSig"] = cmb_sigs.GetShape();
          std::cout << ">> Doing postfit (no uncertainties): TotalProcs" << std::endl;
          post_shapes_tot["TotalProcs"] = cmb.cp().GetShape();
    ...
    for (auto bin : bins) {
      ...
      for (auto proc : cmb_bin.process_set()) {
        ...
        if (skip_proc_errs) {
          std::cout << ">> Doing postfit (no uncertainties): " << bin << "," << proc << std::endl;
      ...
      if (skip_uncertainties) {
          std::cout << ">> Doing postfit (no uncertainties): " << bin << "," << "TotalBkg" << std::endl;
          post_shapes[bin]["TotalBkg"] = cmb_bkgs.GetShape();
          std::cout << ">> Doing postfit (no uncertainties): " << bin << "," << "TotalSig" << std::endl;
          post_shapes[bin]["TotalSig"] = cmb_sigs.GetShape();
          std::cout << ">> Doing postfit (no uncertainties): " << bin << "," << "TotalProcs" << std::endl;
          post_shapes[bin]["TotalProcs"] = cmb_bin.cp().GetShape();
